...
def torch_moe(input, w1, w2, gating_weight, topk, num_expert_group, topk_group):
    ...
    # print(f"++++++++++ input shape : {input.shape}, gating_weight shape: {gating_weight.shape}")
    # print(f"++++++++++ input : {input.shape}, input content:")
    # for i in range(min(5, input.shape[0])):
    #     print(f"  Row {i}: {input[i][:10]}...")
        
    score = torch.nn.functional.linear(input.type(compute_dtype), gating_weight.type(compute_dtype), None)
    # print(f"++++++++++ score : {score.shape}, score content:")
    # for i in range(min(5, score.shape[0])):
    #     print(f"  Row {i}: {score[i][:10]}...")
    ...
    # print(f"++++++++++ topk_weights : {topk_weights.shape}, topk_weights content:")
    # for i in range(min(5, topk_weights.shape[0])):
    #     print(f"  Row {i}: {topk_weights[i][:10]}...")
...
def subprocess_moe(input_path, w1_q_path, w2_q_path, w1_scale_path, w2_scale_path, gating_weight_path, correction_bias_path, topk, num_expert_group, topk_group, ep_rank, ep_size):
    dtype = torch.bfloat16
    ...
    w1_scale = torch.load(w1_scale_path, weights_only=True).cuda() if w1_scale_path else None
    w2_scale = torch.load(w2_scale_path, weights_only=True).cuda() if w2_scale_path else None
    gating_weight = torch.load(gating_weight_path, weights_only=True).cuda()
    correction_bias = torch.load(correction_bias_path, weights_only=True).cuda() if correction_bias_path else None
    ...
    print(f"********* before w1_q shape: {w1_q.shape}")
    print(f"********* before w2_q shape: {w2_q.shape}")
    w1_q = shuffle_weight(w1_q)
    w2_q = shuffle_weight(w2_q)
    print(f"********* after shuffle w1_q shape: {w1_q.shape}")
    print(f"********* after shuffle w2_q shape: {w2_q.shape}")
    ...
    # init device
    rocm_ffn_dispatch = torch.classes.unittest.ROCmFFNDispatchTest(ep_rank, ep_size)


    output = torch.empty_like(input, dtype=dtype, device='cuda')
    rocm_ffn_dispatch.testMoeFFnLayer(
        input,
        w1_q,
        w2_q,
        gating_weight,
    ...
    print("rocm_ffn_dispatch output: ", output)
...
class TestROCmFfnMoe(unittest.TestCase):
    ...
    def _test_moe(self, token, model_dim, inter_dim, num_expert, num_shared_expert, topk, num_expert_group, topk_group, norm_topk_prob, ep_size, dtype):
        ...
        input_file = tempfile.NamedTemporaryFile(prefix='rocm_ffn_moe_test_input_', suffix='.pt')
        w1_file = tempfile.NamedTemporaryFile(prefix='rocm_ffn_moe_test_w1_q_', suffix='.pt')
        w2_file = tempfile.NamedTemporaryFile(prefix='rocm_ffn_moe_test_w2_q_', suffix='.pt')
        gating_weight_file = tempfile.NamedTemporaryFile(prefix='rocm_ffn_moe_test_gating_weight_', suffix='.pt')
        ...
        for ep_rank in range(ep_size):
            ...
            proc = mp.Process(target=subprocess_moe, args=(
                input_file.name,
                w1_file.name,
                w2_file.name,
                None,
